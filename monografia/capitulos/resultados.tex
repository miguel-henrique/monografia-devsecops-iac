...
\scriptsize
\begin{tabular}{|L{4.0cm}|L{2.4cm}|L{2.6cm}|L{5.0cm}|}
\hline
\textbf{Caso de teste} & \textbf{Ferramenta / Gate} & \textbf{Comportamento observado} & \textbf{Justificativa} \\
\hline
Bucket S3 sem SSE & OPA + Checkov & Pipeline interrompida no gate 3 (OPA) & Regra \pathtt{deny\_unencrypted\_s3.rego} imprime CIS 2.1.1 no \textit{output} do \textit{conftest} \\
\hline
Bucket S3 sem \pathtt{public\_access\_block} & OPA + Checkov & Pipeline interrompida & Mesma política, cláusula CIS 2.1.5 \\
\hline
SG com SSH 22 para \texttt{0.0.0.0/0} & OPA + Checkov & Pipeline interrompida no gate 3 & \pathtt{deny\_public\_ssh.rego}; CIS 5.2 \\
\hline
SG com RDP 3389 para \texttt{0.0.0.0/0} & OPA & Pipeline interrompida & \pathtt{deny\_public\_ssh.rego} \\
\hline
EC2 sem IMDSv2 & OPA & Pipeline interrompida & \pathtt{enforce\_imdsv2.rego}; CIS 5.6 / EKS Benchmark 4.3 \\
\hline
IAM \texttt{Action="*"} + \texttt{Resource="*"} & OPA & Pipeline interrompida & \pathtt{deny\_admin\_iam.rego}; NIST AC-6 \\
\hline
RDS/EBS sem criptografia & OPA + Checkov & Pipeline interrompida & \pathtt{deny\_unencrypted\_rds\_ebs.rego}; CIS 2.2/2.3 \\
\hline
Deploy fora da \textit{whitelist} de regiões & OPA & Pipeline interrompida & \pathtt{region\_whitelist.rego}; base para LGPD \\
\hline
Família de instância proibida em \texttt{dev} & OPA & Pipeline interrompida & \pathtt{finops\_instance\_types.rego} \\
\hline
Segredo (AWS key) em \texttt{.env} & Gitleaks (+ ggshield) & \texttt{gate 1} bloqueia o \textit{commit}/PR & \pathtt{gitleaks-action@v2} com \pathtt{.gitleaks.toml} \\
\hline
...
\hline
Contêiner K8s \texttt{privileged=true} & OPA (K8s) & Pipeline interrompida & \pathtt{no\_privileged\_containers.rego}; CIS K8s 5.2.1 \\
\hline
Contêiner K8s sem \texttt{limits} & OPA (K8s) & Pipeline interrompida & \pathtt{require\_resource\_limits.rego} \\
\hline
Imagem K8s com tag \texttt{:latest} & OPA (K8s) & Pipeline interrompida & \pathtt{no\_latest\_tag.rego} \\
\hline
\end{tabular}
...
Retomando as métricas definidas na Seção~\ref{sec:metricas}, os principais achados foram, sintetizados também na Figura~\ref{fig:pipeline}.
