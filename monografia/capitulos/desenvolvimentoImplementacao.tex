\chapter{Modelo Proposto e Provas de Conceito}

Este capítulo apresenta a materialização do modelo proposto por meio de duas Provas de Conceito (PoC) complementares. O modelo conceitual é formalizado na Seção~\ref{sec:modelo} e, em seguida, instanciado em dois substratos técnicos distintos que se validam mutuamente. A escolha por duas PoC responde a dois objetivos distintos e igualmente relevantes:
...
A PoC AWS materializa o modelo em uma conta gerenciada pelo autor, com governança centralizada via AWS Organizations e provisionamento 100\% declarativo em Terraform, conforme demonstrado na Figura~\ref{fig:arquitetura-aws}.

\begin{figure}[!ht]
    \caption{Arquitetura de referência da PoC AWS: AWS Organizations com SCPs de governança, VPC (10.20.0.0/16) segmentada em três \textit{tiers} (\textit{public}, \textit{private-app}, \textit{private-db}), EKS com IRSA e \textit{envelope encryption} via KMS, RDS PostgreSQL multi-AZ criptografado, \textit{shared services} (ECR, S3, KMS, Secrets Manager, IAM) e camada de auditoria contínua (CloudTrail, AWS Config, GuardDuty, Security Hub). O fluxo de credenciais entre GitHub Actions e a AWS é \textit{keyless} via OIDC.}
    \label{fig:arquitetura-aws}
    \centering
    % Extensao omitida propositalmente: se houver .pdf, o pdflatex prefere a
    % versao vetorial; caso contrario usa a .png. Basta exportar o mesmo nome
    % do drawio como PDF para migrar sem alterar este arquivo.
    \includegraphics[width=\textwidth]{Arquivos/figuras/Figura_Arquitetura_PoC_DevSecOps_Arquitetura_AWS}
    \fonte{Elaborado pelo autor com base em \citeonline{PocDevSecOpsAWS2026}.}
\end{figure}

A Figura~\ref{fig:arquitetura-aws} apresenta a \textbf{arquitetura de referência} adotada: da entrada de tráfego (usuários finais) até a persistência (RDS PostgreSQL), passando pela segmentação em três \textit{tiers} de sub-rede, pela integração \textit{keyless} com GitHub Actions via OIDC e pela camada transversal de observabilidade (CloudTrail, AWS Config, GuardDuty, Security Hub).

Os números indicados na figura correspondem ao caminho seguido por uma requisição do usuário final e ao fluxo paralelo de credenciais/imagens: (1) o tráfego HTTPS entra pelo Internet Gateway e é inspecionado pelo AWS WAFv2 (regras OWASP + \textit{rate limit}) antes de chegar (2) ao \textit{Application Load Balancer} nas sub-redes públicas; (3) o ALB encaminha o tráfego interno para os \textit{pods} do EKS nas sub-redes privadas de aplicação, que se conectam (4) ao RDS PostgreSQL em sub-redes privadas de dados via TLS. Do lado do fluxo \textit{keyless}, (5) os \textit{pods} obtêm credenciais efêmeras da AWS por meio do IRSA (\textit{IAM Roles for Service Accounts}); (6) o GitHub Actions se autentica na AWS via OIDC, sem chaves de longa duração no repositório; e (7) as imagens de contêiner produzidas na esteira são publicadas no ECR e, na sequência, consumidas pelo EKS. Todo o perímetro é envolvido por uma camada de \textbf{governança e auditoria}: SCPs no nível da \textit{Organization}, VPC Flow Logs habilitados, CloudTrail multi-região e Security Hub agregando os \textit{findings} do AWS Config (CIS Conformance Pack) e do GuardDuty. As próximas subseções detalham cada uma dessas camadas, começando pelos recursos provisionados.
...
O Quadro~\ref{lst:aws-envdev} mostra a composição do ambiente \texttt{dev} da PoC AWS, no qual cada módulo é uma unidade coesa e isolada. Observa-se que a rede é a primeira dependência, seguida por segurança, IAM, dados e, por fim, computação (EKS) -- a mesma ordem de resolução indicada no cabeçalho do quadro. Esse encadeamento explicita as fronteiras de responsabilidade entre módulos e serve como \textit{blueprint} reutilizável para o ambiente \texttt{prod}, que difere apenas em parâmetros (CIDR, tamanho dos \textit{node groups}, retenção de \textit{logs}), preservando exatamente a mesma topologia de segurança.
...
O módulo \texttt{storage} materializa quatro controles do CIS AWS Foundations em uma única definição declarativa: (i) \pathtt{aws\_s3\_bucket\_public\_access\_block} bloqueando qualquer forma de acesso público (CIS 2.1.5); (ii) \pathtt{aws\_s3\_bucket\_server\_side\_encryption\_configuration} com SSE-KMS obrigatório (CIS 2.1.1); (iii) \pathtt{aws\_s3\_bucket\_versioning} para objetos sensíveis; e (iv) políticas de bucket que negam tráfego não criptografado (\texttt{aws:SecureTransport = false}). O Quadro~\ref{lst:aws-s3} apresenta os dois primeiros controles, aplicados de forma uniforme a todos os \textit{buckets} do ambiente por meio do \texttt{for\_each} sobre \texttt{local.buckets} -- eliminando a possibilidade de um \textit{bucket} ser esquecido ou ficar fora do padrão.
...
A esteira da PoC AWS é organizada em quatro camadas temporais bem definidas, cada uma acionando \textit{gates} de segurança distintos e complementares. A Figura~\ref{fig:pipeline-aws} apresenta essa organização em uma perspectiva \textit{Shift-Left}: a maior parte dos controles é executada \textbf{antes} do código chegar à AWS -- localmente no ambiente do desenvolvedor (segundos) e na CI a cada \textit{pull request} (minutos) --, restando à camada de \textit{runtime} atuar como \textit{guardrail} contínuo para desvios que porventura escapem das camadas anteriores.

\begin{figure}[!htbp]
    \caption[Pipeline DevSecOps da PoC AWS em quatro camadas.]{Pipeline DevSecOps da PoC AWS organizada em quatro camadas: (1) \textit{Pre-commit local} -- \textit{gates} executados em segundos na máquina do desenvolvedor (gitleaks, \texttt{terraform fmt/validate/tflint}, Checkov, Conftest/OPA, hadolint, Trivy \textit{filesystem}); (2) \textit{GitHub Actions CI} -- \textit{gates} executados em minutos a cada \textit{pull request} (\texttt{security-scan}, \texttt{terraform-plan}, \texttt{container-build}, \texttt{terraform-apply}, \texttt{k8s-deploy}); (3) \textit{Runtime AWS} -- \textit{guardrails} contínuos (SCPs, AWS Config + CIS Conformance Pack, GuardDuty, Security Hub, OPA Gatekeeper no cluster EKS, CloudTrail multi-região); e (4) \textit{Resultado mensurável} -- indicadores de conformidade e ganho operacional face a um cenário sem \textit{Shift-Left}.}
    \label{fig:pipeline-aws}
    \centering
    \includegraphics[width=\textwidth]{Arquivos/figuras/Figura_Pipeline_PoC_DevSecOps_AWS}
    \fonte{Elaborado pelo autor com base em \citeonline{PocDevSecOpsAWS2026}.}
\end{figure}

A materialização dessas camadas ocorre em um único arquivo YAML da CI (\texttt{.github/workflows/security-scan.yml}), que implementa os cinco \textit{quality gates} descritos na Seção~\ref{sec:modelo}. Cada \textit{job} depende do anterior (\texttt{needs:}), o que garante o comportamento \textit{Fail-Fast}: se \textbf{secrets} falhar, os demais nem chegam a executar. O Quadro~\ref{lst:aws-pipeline} apresenta os \textit{jobs} de detecção de segredos e de análise estática de Terraform.
...
Diferentemente do Checkov -- que aplica regras genéricas ao HCL --, o OPA analisa o \textbf{plano de execução} do Terraform já resolvido (o JSON de \texttt{terraform show}). Isso torna as políticas capazes de raciocinar sobre valores computados, dependências e o resultado final da alteração. A política do Quadro~\ref{lst:opa-ssh} bloqueia qualquer \textit{Security Group} que exponha SSH (porta 22) ou RDP (porta 3389) para \texttt{0.0.0.0/0}, cobrindo os dois formatos suportados pelo \textit{provider} AWS: \texttt{aws\_vpc\_security\_group\_ingress\_rule} (recurso independente) e \texttt{aws\_security\_group} com blocos \texttt{ingress} \textit{inline}. Note que a mensagem retornada por \texttt{deny} referencia explicitamente o controle CIS AWS 5.2, o que aparece na saída do \texttt{conftest} e facilita a rastreabilidade entre política executada e norma auditável.
...
Outra política, apresentada no Quadro~\ref{lst:opa-finops}, é dedicada à governança financeira (\textit{FinOps}). Ela bloqueia famílias de instância caras (GPU, memória extra-alta, \textit{dedicated hosts} e tamanhos $\geq$ \texttt{24xlarge}) em ambientes não-produtivos, aplicando o conceito de \textit{governance-as-code}~\citeonline{OPA2024}. Trata-se de uma extensão do princípio de \textit{Shift-Left} para uma dimensão que raramente aparece em \textit{benchmarks} tradicionais de segurança -- custo operacional -- mas que é igualmente auditável e passível de bloqueio automático antes do \texttt{terraform apply}, evitando surpresas na fatura sem depender de vigilância humana:
...
O Quadro~\ref{lst:k8s-netpol} apresenta a materialização da primeira dessas medidas. O primeiro manifesto (\texttt{default-deny-all}) fecha todo o tráfego de \textit{ingress} e \textit{egress} do \textit{namespace} \texttt{app}; a partir dele, o segundo manifesto (\texttt{allow-backend-egress}) reabre explicitamente apenas o mínimo necessário: \textit{ingress} vindo do \textit{frontend nginx} na porta 8000 e \textit{egress} para o CIDR do RDS na porta 5432. Esse desenho garante que qualquer \textit{pod} novo, ao entrar no \textit{namespace}, esteja em \textit{deny-by-default} até que uma \textit{NetworkPolicy} específica autorize o seu tráfego -- eliminando a classe de vulnerabilidades em que \textit{workloads} não previstos passam a se comunicar livremente com serviços internos.
...
Complementarmente às \textit{NetworkPolicies}, o Quadro~\ref{lst:k8s-priv} apresenta uma política OPA que bloqueia \textit{containers privileged} e \textit{allowPrivilegeEscalation} -- padrões proscritos pelo CIS K8s Benchmark 5.2.1 e 5.2.5. A política é aplicada tanto na CI (via Conftest sobre os manifestos \texttt{.yaml}) quanto no \textit{runtime}, através do OPA Gatekeeper instalado no \textit{cluster}. Essa dupla execução -- CI + \textit{admission control} -- garante \textit{defense-in-depth}: mesmo que um manifesto seja aplicado fora da esteira (\textit{via kubectl apply} manual), o \textit{admission controller} intercepta e nega o recurso antes de ele existir no \textit{cluster}.
...
A PoC de contêineres foi projetada para ser provisionada com um único comando, o que reforça a proposta de reprodutibilidade. O Quadro~\ref{lst:docker-run} apresenta o roteiro completo -- do \textit{provisionamento} inicial ao \textit{teardown} --, viabilizando que qualquer avaliador reproduza integralmente o cenário local em minutos, a partir apenas de uma máquina com Docker e Terraform instalados.

\begin{lstlisting}[language=bash,caption={Provisionamento e teardown completos da PoC local.},label={lst:docker-run}]
