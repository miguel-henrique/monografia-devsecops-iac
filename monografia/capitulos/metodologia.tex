...
\begin{enumerate}
    ...
    \item \textbf{Definição do cenário e seleção de ferramentas:} identificação dos recursos críticos de infraestrutura (VPC, EKS, RDS, S3, IAM, KMS) e seleção do \textit{stack} tecnológico capaz de suportar a automação de segurança em duas topologias -- nuvem pública e local. A escolha das ferramentas seguiu três critérios objetivos: (i) \textbf{maturidade e adoção industrial} evidenciada por \textit{benchmarks} públicos; (ii) \textbf{cobertura direta} dos controles do CIS Benchmarks e do NIST; e (iii) \textbf{gratuidade ou disponibilidade em camada \textit{open source}}, para preservar a reprodutibilidade acadêmica. O mapeamento resultante foi: \textit{SAST-IaC} com Checkov e TFLint; \textit{Policy-as-Code} com OPA/Conftest; detecção de segredos em duas camadas com Gitleaks (OSS) e GitGuardian/ggshield (SaaS); segurança de contêineres com Trivy (varredura de vulnerabilidades) e Hadolint (\textit{lint} de \textit{Dockerfile}); provisionamento com Terraform 1.9.x; e CI/CD com GitHub Actions autenticado à AWS via OIDC.

    \item \textbf{Proposta do modelo:} elaboração do modelo conceitual de integração DevSecOps + IaC, contemplando as etapas \textit{Source}, \textit{Build}, \textit{Test}, \textit{Deploy} e \textit{Operate}, e a distribuição das ferramentas em cada etapa. A distribuição adotada foi: em \textit{Source}, \textit{Pre-commit Hooks} com \texttt{gitleaks protect} e \texttt{terraform fmt}; em \textit{Build}, análise estática (Checkov, TFLint, Hadolint) e detecção de segredos na CI (Gitleaks + ggshield); em \textit{Test}, Policy-as-Code via OPA/Conftest sobre o JSON do \texttt{terraform plan} e sobre manifestos Kubernetes; em \textit{Deploy}, \texttt{terraform apply} condicionado à aprovação de todos os \textit{gates} anteriores; e em \textit{Operate}, controles contínuos apenas na PoC AWS (CloudTrail, AWS Config com \textit{CIS Conformance Pack}, GuardDuty, Security Hub e OPA Gatekeeper no cluster EKS).
...
Diferentemente de uma inspeção genérica, cada caso de teste possui três elementos formais definidos \emph{a priori}: um \textbf{controle} associado a um \textit{framework} reconhecido (CIS ou NIST); uma \textbf{política executável} (identificada por um \textit{Checkov ID} ou por uma regra OPA/Rego); e o \textbf{resultado esperado} da esteira. A Tabela~\ref{tab:testes-controlados} consolida os casos executados nas duas PoC.
...
\footnotesize
\begin{tabular}{|L{4.2cm}|L{2.6cm}|L{4.4cm}|L{2.8cm}|}
\hline
\textbf{Cenário de teste} & \textbf{Controle} & \textbf{Política aplicada} & \textbf{Ferramenta} \\
\hline
Bucket S3 sem SSE (\textit{aws\_s3\_bucket} sem \pathtt{server\_side\_encryption\_configuration}) & CIS AWS 2.1.1 & \pathtt{deny\_unencrypted\_s3.rego}; CKV\_AWS\_19 & OPA + Checkov \\
\hline
Bucket S3 sem \pathtt{public\_access\_block} & CIS AWS 2.1.5 & \pathtt{deny\_unencrypted\_s3.rego} & OPA + Checkov \\
\hline
\textit{Security Group} liberando SSH (22) para \texttt{0.0.0.0/0} & CIS AWS 5.2 & \pathtt{deny\_public\_ssh.rego}; CKV\_AWS\_24 & OPA + Checkov \\
\hline
\textit{Security Group} liberando RDP (3389) para \texttt{0.0.0.0/0} & CIS AWS 5.2 & \pathtt{deny\_public\_ssh.rego} & OPA \\
\hline
Instância EC2 sem IMDSv2 & CIS AWS 5.6 & \pathtt{enforce\_imdsv2.rego} & OPA \\
\hline
Política IAM concedendo \texttt{Action="*"} + \texttt{Resource="*"} & NIST SP 800-53 AC-6 & \pathtt{deny\_admin\_iam.rego} & OPA \\
\hline
RDS/EBS sem criptografia em repouso & CIS AWS 2.3 / 2.2 & \pathtt{deny\_unencrypted\_rds\_ebs.rego} & OPA \\
\hline
\textit{Deploy} fora da região permitida (\textit{Whitelist}) & Governança / LGPD & \pathtt{region\_whitelist.rego} & OPA \\
\hline
Instância de família proibida em ambiente \texttt{dev} & FinOps & \pathtt{finops\_instance\_types.rego} & OPA \\
\hline
Segredo (chave AWS) versionado em \texttt{.env} & OWASP -- \textit{Cryptographic Failures} & Regras padrão + \pathtt{.gitleaks.toml} & Gitleaks + GitGuardian \\
\hline
...
\hline
Contêiner privileged=true & CIS K8s 5.2.1 & \pathtt{no\_privileged\_containers.rego} & OPA (K8s) \\
\hline
Contêiner sem \texttt{resources.limits} & CIS K8s 5.x / FinOps & \pathtt{require\_resource\_limits.rego} & OPA (K8s) \\
\hline
Imagem com tag \texttt{:latest} & Boas práticas & \pathtt{no\_latest\_tag.rego} & OPA (K8s) \\
\hline
\end{tabular}
